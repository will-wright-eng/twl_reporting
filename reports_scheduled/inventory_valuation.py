...
def run():
    ...
    overlap, _, _ = lists_calc(list(df.product_code_sku),
                               list(dfs[1].custom_sku))
    overlap.remove('MVP-147')  # duplicate in lightspeed df
    skus = pd.DataFrame(overlap)
    skus.columns = ['sku']
    df_ls = skus.merge(dfs[1],
                       left_on='sku',
                       right_on='custom_sku',
                       how='left')
    ...
    df = df.merge(df_ls,
                  how='left',
                  left_on='product_code_sku',
                  right_on='sku')

    # clean dollar amounts
    cols = [i for i in list(df) if 'price' in i
            ] + [i for i in list(df) if '_cost' in i]
    for col in cols:
    ...
    df[cols_dict['cost']] = df.apply(lambda x: bool_fxn(
        x['cost_price'], x['default_cost'], x['cost_price_null']),
                                     axis=1)
    ...
    df[cols_dict['inv_val_cost']] = df[cols_dict['cost']] * df[
        cols_dict['stock']]
